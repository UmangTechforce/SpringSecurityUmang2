...
public class EmployeeController {
	 ...
	 */
	@PostMapping("/login")
	public ResponseEntity<Object> login(@RequestBody final LoginRequestDTO loginRequestDTO) throws InvalidKeyException, NoSuchAlgorithmException {
		...
		return new ResponseHandler.ResponseBuilder().setData(loginResponseDTO).setStatus(HttpStatus.ACCEPTED)
				.setMessage(messageSource.getMessage("employee.login",null, null))
				.build()
				.create();
	}
	...
	public ResponseEntity<Object> updateEmployee(@Valid @RequestBody final EmployeeRequstDTO employeeRequstDTO){
		...
		return new ResponseHandler.ResponseBuilder().setData(employeeResponseDTO).setMessage(messageSource.getMessage("employee.update", null, null))
	...
	public ResponseEntity<Object> getEmployee(@PathVariable final Long id , Principal principal){
		...
		return new ResponseHandler.ResponseBuilder().setData(employeeResponseDTO)
				.setMessage(messageSource.getMessage("employee.present",null, null))
				.setStatus(HttpStatus.ACCEPTED)
				.build()
				.create();
	}
